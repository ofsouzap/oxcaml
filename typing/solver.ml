...
module Solver_mono (Hint : Hint) (C : Lattices_mono) = struct
  ...
  let zap_to_ceil : type a l. a C.obj -> (a, l * allowed) mode -> log:_ -> a =
   fun obj m ~log ->
    ...
    assert (
      submode obj (Amode (ceil, Const Hint.const_none)) m ~log |> Result.is_ok);
    ceil
    ...
    let rec loop lower lower_hint =
      ...
      | Error (a, a_hint) ->
        ...
        loop (C.join obj a lower)
          (hint_join_const obj a a_hint lower lower_hint)
    in
    ...
    | Amodejoin (a, _a_hint, mvs) ->
      ...
      VarMap.iter
        (fun _ mv ->
          ...
          assert (
            submode_mvc obj mv floor (Const Hint.const_none) ~log
            |> Result.is_ok))
        mvs;
      floor
