...
module Solver_mono (H : Hint) (C : Lattices_mono) = struct
  ...
  let submode_cmv :
   ...
   fun ~log obj a a_hint (Amorphvar (v, f, f_hint) as mv) ->
    ...
    then Error (mupper, mupper_hint)
      ...
      assert (Result.is_ok (submode_cv ~log src a' a'_hint v));
      Ok ()
  ...
  let zap_to_ceil : type a l. a C.obj -> (a, l * allowed) mode -> log:_ -> a =
   fun obj m ~log ->
    ...
    (* Since this call to [submode] should always succeed,
       we provide a throwaway, arbitrary hint to [Amode] *)
    assert (
      submode obj (Amode (ceil, Const H.const_none)) m ~log |> Result.is_ok);
    ceil
      ...
      VarMap.iter
        (fun _ mv ->
          (* Since this call to [submode_mvc] should always succeed,
             we provide a throwaway, arbitrary hint *)
          assert (
            submode_mvc obj mv floor (Const H.const_none) ~log |> Result.is_ok))
        mvs;
      floor
  ...
  let newvar obj = Amodevar (Amorphvar (fresh obj, C.id, H.morph_skip))
  ...
  let newvar_below (type a l_ l r) (obj : a C.obj) (m : (a, l_ * allowed) mode)
    ...
    | Amodevar mv ->
      let u = fresh obj in
      let mu = Amorphvar (u, C.id, H.morph_skip) in
      assert (Result.is_ok (submode_mvmv obj ~log:None mu mv));
      allow_left (Amodevar mu), true
    | Amodemeet (a, a_hint, mvs) ->
      let u = fresh obj in
      let mu = Amorphvar (u, C.id, H.morph_skip) in
      assert (Result.is_ok (submode_mvc obj ~log:None mu a a_hint));
      VarMap.iter
